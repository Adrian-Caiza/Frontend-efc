...
const CrudView = ({ moduleName, endpointName, tableColumns, formFields, hideEdit }) => {
  ...
  const handleLogout = () => {
    localStorage.removeItem('userToken'); localStorage.removeItem('userName'); localStorage.removeItem('userImage');
    navigate('/login');
  };
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-8 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
          <Link to="/materias" className={getNavClass('/materias')}>📚 Materias</Link>
          <Link to="/estudiantes" className={getNavClass('/estudiantes')}>🎓 Estudiantes</Link>
          <Link to="/matriculas" className={getNavClass('/matriculas')}>📝 Matrículas</Link>
        </div>
